...
fn sm3Hash(mut cx: FunctionContext) -> JsResult<JsString> {
    ...
    let dataString = cx.argument::<JsString>(0)?.value(&mut cx);
    let hash =  SM3::hash(&dataString.into_bytes());
...
impl SM2 {
    ...
    pub fn encrypt(data: &Vec<u8>, pubKey: &Vec<u8>) -> Result<Vec<u8>, String>  {
        ...
        unsafe {
            ...
            let evp_key = match SM2::create_evp_pkey(pubKey, 1) {
                Ok(evp_key) => evp_key,
                Err(e) => return Err(e),
            };
    ...
    pub fn decrypt(data: &Vec<u8>, priKey: &Vec<u8>) -> Result<Vec<u8>, String> {
        ...
        unsafe {
            let ciphertext_len: *mut size_t = Box::into_raw(Box::new(0));
            let mut pkey = match SM2::create_evp_pkey(priKey, -1) {
                Ok(evp_key) => evp_key,
                Err(e) => return Err(e),
            };
    ...
    pub fn sign(data: &Vec<u8>, priKey: &Vec<u8>) -> Result<Vec<u8>, String> {
        ...
        unsafe {
            ...
            let mut pkey = match SM2::create_evp_pkey(priKey, 0) {
                Ok(evp_key) => evp_key,
                Err(e) => return Err(e),
            };
