...
local gmC = require "gmCryptor-openssl"

 
local sm3Data = gmC.sm3Hash(testStr)
...
local enData = gmC.sm4EcbEncrypt(testStr, sm4KeyStr)

local starttime = os.clock();
for i = 1, N do 
    gmC.sm4EcbEncrypt(testStr, sm4KeyStr) 
end
print(string.format(string.toleft("SM4[ECB]加密["..#enData.."位]",30)..": %.4f", os.clock() - starttime) .. "  " ..  enData);

local deData = gmC.sm4EcbDecrypt(enData,sm4KeyStr)

local starttime = os.clock(); 
for i = 1, N do
  gmC.sm4EcbDecrypt(enData,sm4KeyStr) 
end

print(string.format(string.toleft("SM4[ECB]解密["..#deData.."位]",30)..": %.4f", os.clock() - starttime).. "  ".. deData);

local enData = gmC.sm4CbcEncrypt(testStr, sm4KeyStr, sm4IvStr)

local starttime = os.clock();
for i = 1, N do 
    gmC.sm4CbcEncrypt(testStr, sm4KeyStr, sm4IvStr) 
end
print(string.format(string.toleft("SM4[CBC]加密["..#enData.."位]",30)..": %.4f", os.clock() - starttime) .. "  " ..  enData);


local deData =  gmC.sm4CbcDecrypt(enData,sm4KeyStr,sm4IvStr)

local starttime = os.clock(); 
for i = 1, N do
  gmC.sm4CbcDecrypt(enData,sm4KeyStr,sm4IvStr) 
end

print(string.format(string.toleft("SM4[CBC]解密["..#deData.."位]",30)..": %.4f", os.clock() - starttime).. "  ".. deData);

local enData = gmC.sm2Encrypt(testStr, pubStr, 0) 

local starttime = os.clock();
for i = 1, N do 
    gmC.sm2Encrypt(testStr, pubStr, 0) 
end
print(string.format(string.toleft("SM2[C1C3C2]加密["..#enData.."位]",30)..": %.4f", os.clock() - starttime) .. "  " ..  enData);

local deData = gmC.sm2Decrypt(enData,priStr,0)
 
local starttime = os.clock(); 
for i = 1, N do
   gmC.sm2Decrypt(enData,priStr,0)
end
print(string.format(string.toleft("SM2[C1C3C2]解密["..#deData.."位]",30)..": %.4f", os.clock() - starttime).. "  "..deData);

local enData = gmC.sm2Encrypt(testStr, pubStr, 1) 

local starttime = os.clock();
for i = 1, N do 
     gmC.sm2Encrypt(testStr, pubStr, 1) 
end
print(string.format(string.toleft("SM2[C1C2C3]加密["..#enData.."位]",30)..": %.4f", os.clock() - starttime) .. "  " ..  enData);

local deData = gmC.sm2Decrypt(enData,priStr,1)
 
local starttime = os.clock(); 
for i = 1, N do
   gmC.sm2Decrypt(enData,priStr,1)
end
print(string.format(string.toleft("SM2[C1C2C3]解密["..#deData.."位]",30)..": %.4f", os.clock() - starttime).. "  "..deData);

local enData = gmC.sm2EncryptAsn1(testStr, pubStr) 

local starttime = os.clock();
for i = 1, N do 
     gmC.sm2EncryptAsn1(testStr, pubStr) 
end
print(string.format(string.toleft("SM2[Asn1]加密["..#enData.."位]",30)..": %.4f", os.clock() - starttime) .. "  " ..  enData);

local deData = gmC.sm2DecryptAsn1(enData,priStr)
 
local starttime = os.clock(); 
for i = 1, N do
   gmC.sm2DecryptAsn1(enData,priStr)
end
print(string.format(string.toleft("SM2[Asn1]解密["..#deData.."位]",30)..": %.4f", os.clock() - starttime).. "  "..deData);

local sig = gmC.sm2Signature(testStr,priStr)
 
local starttime = os.clock(); 
for i = 1, N do
   gmC.sm2Signature(testStr,priStr)
end
print(string.format(string.toleft("SM2加签["..#sig.."位]",30)..": %.4f", os.clock() - starttime).. "  "..sig);

local veri =  gmC.sm2VerifySign(testStr,sig,pubStr);

local starttime = os.clock(); 
for i = 1, N do
    gmC.sm2VerifySign(testStr,sig,pubStr);
end
