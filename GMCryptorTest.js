...
var os = require('os');
var rustCryptorWasm = require('./GMCryptorRustWasm/pkg/GMCryptorRustWasm.js')
var rustCryptorAddon = require('./GMCryptorRustAddon/.')
var goCryptorAddon = require('./GMCryptorGoAddon/build/Release/GMCryptorGoAddon-' + os.platform() + "-" + os.arch())
var cCryptorAddon = require('./GMCryptorCAddon/build/Release/GMCryptorCAddon-' + os.platform() + "-" + os.arch())
...
console.log("C-ADDON 测试" + count + "次=================================================");
...
var sm2Enc = doFn('SM2[C1C3C2]加密', cCryptorAddon.sm2Encrypt, testString, sm2PublicKey, 0)
doFn('SM2[C1C3C2]解密', cCryptorAddon.sm2Decrypt, sm2Enc, sm2PrivateKey, 0)
var sm2Enc = doFn('SM2[C1C2C3]加密', cCryptorAddon.sm2Encrypt, testString, sm2PublicKey, 1)
doFn('SM2[C1C2C3]解密', cCryptorAddon.sm2Decrypt, sm2Enc, sm2PrivateKey, 1)
...
console.log("RUST-ADDON 测试" + count + "次=================================================");
...
var sm2Enc = doFn('SM2[C1C3C2]加密', rustCryptorWasm.sm2Encrypt, testString, sm2PublicKey, 0)
doFn('SM2[C1C3C2]解密', rustCryptorWasm.sm2Decrypt, sm2Enc, sm2PrivateKey, 0)
var sm2Enc = doFn('SM2[C1C2C3]加密', rustCryptorWasm.sm2Encrypt, testString, sm2PublicKey, 1)
doFn('SM2[C1C2C3]解密', rustCryptorWasm.sm2Decrypt, sm2Enc, sm2PrivateKey, 1)
...
var libGMCryptor = ffi.Library('./GMCryptor-' + os.platform() + "-" + os.arch(), {
...
(async function () {
  ...
  const {
    instance
  } = await WebAssembly.instantiate(fs.readFileSync('GMCryptor.wasm'), go.importObject)
  var run = go.run(instance)
  console.log("GO-WASM 测试" + count + "次=================================================(性能折中，无内存泄露)");
